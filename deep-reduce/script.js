/* const data = [[{"id":1,"name":"Leanne Graham","username":"Bret","email":"Sincere@april.biz","address":{"street":"Kulas Light","suite":"Apt. 556","city":"Gwenborough","zipcode":"92998-3874","geo":{"lat":"-37.3159","lng":"81.1496"}},"phone":"1-770-736-8031 x56442","website":"hildegard.org","company":{"name":"Romaguera-Crona","catchPhrase":"Multi-layered client-server neural-net","bs":"harness real-time e-markets"}},
...
let arr = data.map(function(element) {
    return {geo:element.Geo * 1};
});

arr = arr.filter(function (element) {return element.geo ===  {"lat":"-37.3159","lng":"81.1496"}
});

let sumGeo = arr.reduce(function (acc, cur) {
    return acc + cur;
});

geo = sumGeo/arr.length;

console.log(geo);

/*const address = data.reduce((total, amount) => {
    amount.geo.forEach( geo => {
        ttal.push(geo);
    })
    return total;
})
...
userGeos = user.reduce((user, {}) => {
    return {userName: user.name, latitude: user.address.geo.lat, longitude: user.address.geo.lng}
});

console.table(userGeos);
